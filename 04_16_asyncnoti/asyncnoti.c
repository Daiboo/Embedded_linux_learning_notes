...
void timer_function(unsigned long arg)
{
    ...
    if(atomic_read(&dev->releasekey))  // 一次完整的按键过程
    {
        if(dev->async_queue)
        {
            kill_fasync(&dev->async_queue, SIGIO, POLLIN);  // 通知应用程序
        }
    }
#if 0
    // 唤醒进程
...
unsigned int imx6uirq_poll(struct file *filp, struct poll_table_struct *wait)
{
    ...
    poll_wait(filp, &dev->r_wait, wait);  // 添加到等待队列
    
    if(atomic_read(&dev->releasekey))  // 有按键按下
    {
        mask = POLLIN | POLLRDNORM;  // 普通或优先级带数据可读
    }
    return mask;
}
...
{
    imx6uirq_fasync(-1, filp, 0);  // 注销异步通知
    return 0;
}
