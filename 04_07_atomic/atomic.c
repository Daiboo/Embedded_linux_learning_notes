...
{
    /* The atomic variable 'lock' is set 1 by default.
    When someone invoke the driver, the 'lock' will be decreased by 1.
    After the operation is done, the 'lock' will be increased by 1.
    If the 'lock' is 0, the driver is busy.
    if the 'lock' is 1, the driver is free.
    */
   if(!atomic_dec_and_test(&gpioled.lock)){
        atomic_inc(&gpioled.lock);  /* increase the lock, the lock is 0*/
        return -EBUSY;
   }
...
{
    /* release the atomic variable lock */
...
static int __init led_init(void)
{
    ...
    /* initialize the atomic variable 'lock' to 1 */
