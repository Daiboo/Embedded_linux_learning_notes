...
{
    unsigned long flags;
    spin_lock_irqsave(&gpioled.spinlock, flags);  /* lock */
    if(gpioled.dev_state)   /* driver is busy */
    {
        spin_unlock_irqrestore(&gpioled.spinlock, flags);  /* unlock */
        return -EBUSY;
    }
    gpioled.dev_state++; 
    spin_unlock_irqrestore(&gpioled.spinlock, flags);  /* unlock */
